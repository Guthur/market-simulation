...
(defn- fill-order
  ...
  and ORDER-LIST"
  [order order-list matching-price-comparator]
...
(defn match-sell-order
  ...
  (let [[order order-list] (fill-order order (:buy-orders order-book) <=)]
    (add-sell-order (make-order-book order-list (:sell-orders order-book))
                    order)))

(defn match-buy-order
  ...
  (let [[order order-list] (fill-order order (:sell-orders order-book) >=)]
    (add-buy-order (make-order-book (:buy-orders order-book) order-list)
                   order)))
